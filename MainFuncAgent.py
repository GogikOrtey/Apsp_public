...
* Глобальне
    * Нужно добавлять host в массив данных, когда я его извлекаю в extraction_selectors
        * Bkb вставлять и извлекать его на этапе генерации входных данных - будет лучше
...
* parsePage
    * Обработку извлечения пагинации как номера страниц - проверить на других примерах
    * [Возможно позже] Составить универсальный запрос к ИИ, что бы он извлекал нужные параметрв
      для текста запроса, пагинации и оффсета, если он есть
      И попробовать заменить 

* parseCard
    ☑ Предварительно закончил
...
Текущие задачи:
    ...
    * Добавить ещё 8 примеров простых сайтов, и убедиться что генерация работает корректно
        * Таких примеров, на которых всё бы работало корректно
    
    * Наверное всё таки попробовать выписат запрос, что бы ИИ собирал ссылку для parsePage
        * Там в целом то надо только примеры ему скинуть, парочку
        * Либо добавить обработку параметов в пути URL а не только в параметрах

    * Проверить, где я использую get_html без кеша

    * Также в логе выводить кол-во обращений к ИИ
