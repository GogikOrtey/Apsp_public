...
С каким конкретно полем тебе нужно работать сейчас - указано в глобальном плане задачи (main_plan), а также в блоке "Твой фокус прямо сейчас". Для этого поля:
...
Если ты увидишь, что на одной из ссылок написанный тобой код извлекает неверное значение, то пометь ячейку field__[название поля]__ok_on_[номер ссылки]_link в result как false, а в остальные 2 ячейки других страниц - запиши None (что бы не забыть потом проверить код на них снова). 
...
Далее, тебе нужно будет перейти на один шаг назад, используя goto_main_plan_step, что бы изменить и уточнить код для извлечения значений этого поля. Но перед ипользованием goto_main_plan_step самостоятельно очисти значение из ячейки field__[название поля]__code_gen_completed текущего поля, что бы когда ты выполнишь переход назад, значение ячейки field__[название поля]__code не сбросилось. И тогда при использовании goto_main_plan_step пропиши вргемент clear_fills = false
