...
Т.е. на этом этапе тебе нужно будет отправить в этот инструмент код, который записан в field__[название поля]__code, текущего поля с которым ты работаешь (указано в глобальном плане задачи (main_plan), и в блоке "Твой фокус прямо сейчас"). Этот код выполнится там, на переданных страницах, и вернёт результат - на каждую страницу в массиве:
[
    {
...
Тебе нужно будет проанализировать, что полученные значения для каждой страницы - валидны, корректны, не пусты, и соответствуют семантике поля.
...
- Если значение main_result содержит что-то лишнее, например для поля article извлекается строка "Артикул: 15487", то значит его нужно будет просто очистить далее в написании кода (что бы в итоге получилось "15487"). Пометь это в memory.
...
Когда верный селектор будет найден, просмотри результаты ##########, если нужна быдет какая-то дополнительная обработка значения - зафиксируй это в memory, и переходи далее к шагу генерации кода для этого поля.
...
Когда код для извлечения значений будет составлен, переходи к 3 этапу - его валидации:

————————————————————————

3. ВАЛИДАЦИЯ НАПИСАННОГО КОДА

Здесь тебе нужно будет запустить написанный код, для извлечения значений поля. Этот код уже будет записан в ячейке field__[название поля]__code в result. Для валидации используй инструмент ########

Ему в аргументах - передай код, который записан в ячейке field__[название поля]__code в result для текущего поля. Этот инструмент вернёт тебе значения для всех переменных, которые были инициализированы в твоём переданном фрагменте. Т.е. тебе не нужно передавать в него дополнительно `console.log()` и подобное, он сам вернёт все значения инициализирвоанных переменных. Т.е. ожидается, что ты отправишь ему код, который извлекает значение для одного указанного поля. 
...
2.1 Используй инструмент ########## для проверки кода обработки текущего поля. 
