...
- **Основные страницы** (см. `Apsp_front/app.py` и `Apsp_front/templates/`):
  - `main_page_1`: ввод URL сайта (добавлен счётчик активных задач в углу). При POST проверяет наличие готовых результатов для введённого URL. Содержит ссылку на `/all_tasks`.
