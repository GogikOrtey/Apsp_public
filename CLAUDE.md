...
- Также при старте фронта выполняется reconcile "осиротевших" задач: если в `meta.json` осталось `runtime_status="running"` (процесс был перезапущен/упал во время генерации), такая задача автоматически переводится в `FAILED` с понятной причиной, чтобы UI не зависал и счётчик активных задач был корректным.
