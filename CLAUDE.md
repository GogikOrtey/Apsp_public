...
- При старте Flask-фронта (`Apsp_front/app.py`) также выполняется автоподхват “висящих” задач прошлого процесса:
  ...
  - политика попыток: ориентируемся на `meta.attempts` — **1 раз старт + 1 раз перезапуск** (итого `attempts<=2`)
  - если `attempts>=2` и задача всё ещё `WORK`, она переводится в `FAILED` с причиной “падение сервиса” (чтобы UI/скачивание/Telegram были предсказуемыми)
...
- Если после рестарта сервиса найдена “висящая” задача (`status=WORK` и `last_started_at_ts < APP_START_TIME`) и она отправляется на перезапуск (attempts < 2) — пользователю приходит отдельное сообщение-заглушка, что сервис перезапущен и задача начнёт выполняться заново, со ссылкой на `main_page_2/<uid>`.
