...
## Важное:
- Количество попыток на перезапуск задачи при падении с ошибкой задаётся в task_registry.py, по умолчанию = 3
...
- Реестр задач: `task_runtime/task_registry.py` (`TaskRegistry`).
  ...
  - При ошибке пайплайна делается до **3 попыток** суммарно (1 запуск + 2 ретрая). Только после исчерпания попыток задача становится `FAILED`.
