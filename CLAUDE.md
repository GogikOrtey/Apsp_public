...
- **`GET|POST /select_fields`**: страница выбора полей для парсинга. Пользователь может выбрать, какие поля он хочет извлекать (название, цена, рейтинг и т.д.). Обязательные поля (name, link, price, stock, timestamp) заблокированы и всегда выбраны. При сохранении выбор сохраняется в cookie `selected_fields` и пользователь возвращается на `main_page_1`. Доступ: кнопка "Уточнить поля" на `main_page_1`.
