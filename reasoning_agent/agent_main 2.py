...
Всё собрать в этот список задач
steps_future - надо чистить из истории (и видимо другие поля тоже)
про development_feedback - нигде не написано
Надо выводить системный промпт в начале лога (для себя)
development_report добавить
Спросить, стоит ли всегда передавать текущую задачу
Спросить у GPT, всё ли верно в логе, и хорошо ли читается. Или может стоит улучшить
    Хорошо ли понятна задача ему например
Возможно выделить в отдельные объекты - действие, которое он запросил на предыдущем шаге
и результат выполнения инструмента, который был
Там результаты добавляются в историю на этот же шаг, в observation. Нужно узнать у GPT, как лучше - добавлять в этот же шаг, добавлять в другой шаг результат, и/или отдельно выделять цель предыдущего шага, инструмент который был использован, и результат, который он вернул?


Типо: На предыдущем шаге ты решил что нужно: target, для этого использовал инструмент action (args), и он вернул результат:

Или: На предыдущем шаге была выделена цель: _ для этого был использован инструмент _ (с аргументами _), который вернул результат: _


Спросить в свободной беседе, используют ли агенты формат рассуждение -> рассуждение, а не только
рассуждение -> действие -> рассуждение
Спросить про большие буквы
...
{
  "type": "<tool_gap | contract_issue | memory_issue | prompt_issue | optimization | other>",
