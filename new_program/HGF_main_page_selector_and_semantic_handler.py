...
2. Сбор семантики:
    ...
    - Отдавай приоритет более точным словам, обозначающим конкретные предметы и собирай меньше общих, описывающих группы и категории товаров. Отдавай приоритет точным, таким как: "дверь", "ванна", "дрель", "линолиум". Избегай общих слов, таких как: "сантехника", "инструмент", "напольные" и пр.
...
3. Обнаружение поля поиска
    ...
    - Если поле поиска отсутствует на странице - верни status="error" и в analysis_message укажи, что поле поиска отсутствует на странице.
        - Например, если явно видно что страница не является интернет-магазином (и отсутствует поле поиска товаров).
