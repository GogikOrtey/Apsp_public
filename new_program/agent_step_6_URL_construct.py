...
1. Внутри этого элемента должна быть ссылка на вторую страницу. Попробуй прямо перейти на неё. Если она не полная (напримр нет схемы и домена), то дополни её что бы она стала валидной.

2. Если внутри этого элемента нет URL, то изучи подробнее окружение элемента pagination_page2_selectors, используя инструмент get_html_frame_on_current_page, либо попробуй два других запасных селектора. Если перехода не происходит и страница не меняется - возвращай FAILED с сообщением, что не удалось перейти на вторую страницу поисковой выдачи.
