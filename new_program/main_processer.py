...
- Реализовать функцию которая вытаскивает html одного товара, по селектору ссылки на товар
- get_max_page_checker реализовать
- Реализовать набор функций-проверялок, которые будут запускать код с переданными частями в Node.JS
    - Для валидвции правильности bзвлечения ссылки на товар, max пагинации и формирования запроса (тут хз).

- Реализовать логику, которая разбивает main_task на несколько, и посылает в агента частями
    + в зависимости от каких-то условий, таких как значение поля в result
- Алгоритм уже есть, надо его записать в коде
...
- Если при выполнении инсутрмента произошла ошибка, она корректно обрабатывается?
...
- Прокрутка страницы вниз до конца 
    - И вверх до начала
    - К указанному селектором элементу
