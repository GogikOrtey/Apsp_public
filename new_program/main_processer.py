...
- Также нужно будет продумать, где и как на этом этапе я буду собирать ссылки для работы генератора parseCard
    ...
    - Надо будет ещё раз запустить агента, и попросить его проверить, верно ли она работает (тут хз, но идея прикольная)
        - Что бы Агент собирал код этой функции, проверял её, и если всё ок, то запускал и она бы собирала эти ссылки уже сама
    - И этим инструментом на автомате собрать 100 ссылок
        - И из них выбирать 3 случайным образом

- Где-то тут также надо будет проверять, есть ли ограничения на сайте, и потребуется ли решалка
    - Проверять так: Извлечь селекторы товара на странице, получить её через curl,
        1. Сравнить, если большие отличия - то abort
        2. Попробовать извлечь данные по тем же селекторам. Если значения отличаются - то тут уже хз

- Затем переходить к генерации parseCard
- Можно сразу попробовать собрать всё вместе, что бы он генерил итоговый код
    - Шаблон для makeRequest останется без изменений, общий шаблон кода тоже
    - Фронт надо будет поправить, но это в самую последнюю очередь
- Попробовать запустить в А-Парсере

- И там уже надо будет смотреть, успеваю ли я допилить логику с проверкой селекторов
    - Доп. обрабатывать точно надо будет. Это в одном проходе агента по всем полям, или нет, можно разными запросами без агента, так даже лучше будет
    - Но речь идёт про проверку и валидацию на разных страницах
- И неограниченное количество полей добавить
    - Надо будет прописать хорошие описания к каждому
    - И доработать промпт генерации parseCard
...
- Как напишу весь текст алгоритма, прогнать его с аннотациями всех моих инструментов. И спросить ChatGPT, хватит ли их для выполнения задачи, или стоит/нужно какие-то добавить
...
- Пофиксить баги из get_html_frame
