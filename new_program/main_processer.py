...
- Реализовать все инструменты для работы с Playwright
    - Надо будет получше расписать что каждй делает и зачем может быть нужен
    - Все функции из Playwright требуют page. Надо это будет убрать
...
def main_processer(input_url):
    ...
    - Если ничего не происходит - проанализируй ситуацию
