...
- Более подробно расписать что каждй делает и зачем может быть нужен в Playwright
- Реализовать инструменты из html_toolkit
- Задание на 2й шаг для агента практически выписал
    - В целом, можно уже даже запускать
- Создать отлельный файл, в котором мы будем запускать агента с нужным входом
