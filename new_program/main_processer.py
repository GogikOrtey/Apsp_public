...
- Проверить, что агент может использовать инструментв из PL, на простом задании
...
def main_processer(input_url):
    ...
    Если не указано иного, то выбирай первые элменты из массивов в input_data.


    ##################
    ###### Изменить - там где инструментв писать инструмент: _, а там где данные, писать данные из поля _

    Алгоритм, что тебе нужно сделать:

    1. Выбрать селектор (из поля search_input_selectors из input_data), проверить что он даёт результат на странице (используя find_elements)
    2. Получить окружающий html код вокруг этого элемента, убедиться что это действительно поле ввода запроса на поиск (используя get_html_frame)
