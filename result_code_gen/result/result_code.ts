...
import {
    toArray, isBadLink,
    name, stock, imageLink, article, category, price, breadCrumbs, series, weight, length, width, height, link, timestamp
...
export class JS_Base_makitaclubru extends JS_Base_Custom {
    ...
    //#region Парсинг поиска
        ...
        if (set.page && set.page > 1) url = new URL(`${HOST}/page/${set.page}/?${url.searchParams.toString()}`)
    ...
    async parseCard(set: SetType, cacher: Cacher<ResultItem[]>) {
        ...
        const stock = $("form.cart button.single_add_to_cart_button")?.first().text().trim()?.includes("В корзину") ? "InStock" : "OutOfStock"
        const imageLink = $(".woocommerce-product-gallery__image img.wp-post-image")?.first()?.attr("data-src") || $(".woocommerce-product-gallery__image img.wp-post-image")?.first()?.attr("data-large_image") || $(".woocommerce-product-gallery__image img.wp-post-image")?.first()?.attr("src") || ""
        const article = $(".product_meta .sku")?.first().text().trim()
        const category = $(".product_meta .posted_in a")?.first().text().trim()
        const price = $(".summary p.price .woocommerce-Price-amount")?.first().text().trim()?.replace(/,/g, ".")?.replace(/[^\d.]/g, "") || ""
        const breadCrumbs = $(".breadcrumbs .woocommerce-breadcrumb, .breadcrumbs nav.woocommerce-breadcrumb")?.first().text().trim()?.replace(/\s*\/\s*/g, "/") || ""
        const seriesText = $(".woocommerce-product-details__short-description, #tab-description, .woocommerce-Tabs-panel--description")?.first().text().trim() || "";
        const series = (seriesText.match(/\b(Серия)\b\s*[:\-–—]?\s*([^\n\r.;,]+)/i)?.at(2)?.trim()) || ""
        const weight = ((() => {
          const rows = $(".woocommerce-Tabs-panel--additional_information table.woocommerce-product-attributes tr.woocommerce-product-attributes-item");
          const row = rows.filter((i, el) => $(el).find("th.woocommerce-product-attributes-item__label").text().trim().toLowerCase().includes("вес")).first();
          return row.find("td.woocommerce-product-attributes-item__value").text().trim()?.replace(/,/g, ".") || "";
        })())
        const length = ((() => {
          const rows = $(".woocommerce-Tabs-panel--additional_information table.woocommerce-product-attributes tr.woocommerce-product-attributes-item");
          const row = rows.filter((i, el) => $(el).find("th.woocommerce-product-attributes-item__label").text().trim().toLowerCase().includes("длина")).first();
          return row.find("td.woocommerce-product-attributes-item__value").text().trim()?.replace(/,/g, ".") || "";
        })())
        const width = ((() => {
          const rows = $(".woocommerce-Tabs-panel--additional_information table.woocommerce-product-attributes tr.woocommerce-product-attributes-item");
          const row = rows.filter((i, el) => $(el).find("th.woocommerce-product-attributes-item__label").text().trim().toLowerCase().includes("ширина")).first();
          return row.find("td.woocommerce-product-attributes-item__value").text().trim()?.replace(/,/g, ".") || "";
        })())
        const height = ((() => {
          const rows = $(".woocommerce-Tabs-panel--additional_information table.woocommerce-product-attributes tr.woocommerce-product-attributes-item");
          const row = rows.filter((i, el) => $(el).find("th.woocommerce-product-attributes-item__label").text().trim().toLowerCase().includes("высота")).first();
          return row.find("td.woocommerce-product-attributes-item__value").text().trim()?.replace(/,/g, ".") || "";
        })())
